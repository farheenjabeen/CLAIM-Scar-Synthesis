...
from ddpm.common_utils.measure import hd, hd_2D_stack, asd, volumesimilarity
import pandas as pd
...
def p_value_test(reference_df, test_df, attributes=['LV_Dice', 'MYO_Dice', 'RV_Dice']):
    '''
    given a reference_df[pandas.dataframe] and test_df[pandas.dataframe] for a specific dataset
...
class runningMySegmentationScore(object):
    ...
    def __init__(self, n_classes, idx2cls_dict=None, metrics_list=['Dice']):
        self.n_classes = n_classes
    ...
    def update(self, pid, preds, gts, voxel_spacing=None):
        ...
        for c, class_name in self.idx2cls_dict.items():
            ...
            for metric in self.metrics:
                ...
                if metric == 'VolSim':
                    score = volumesimilarity(result=pred_c_i.reshape(n, h, w),
    ...
    def reset(self):
        for _, score in self.multi_scores.items():
            self.score = []
        self.tables = []
