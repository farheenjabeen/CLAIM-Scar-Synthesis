    ...
    def train(
        ...
        while self.step < self.train_num_steps:
            for i in range(self.gradient_accumulate_every):
                ...
                if (self.step == 0 or self.step % 10 == 0):
                    self.model.eval()
                ...
                # ---------------------------------------------------------------------------------------
                    ...
                    if (self.step == 0 or self.step % 50 == 0):
                        data = torch.cat((path_data.squeeze(1), gen_path_data.squeeze(1)))
                        data = (data - torch.min(data)) / (torch.max(data) - torch.min(data))
                        mask = torch.cat((path_mask.squeeze(1), gen_path_mask.squeeze(1)))
                        hist = torch.cat((path_hist, gen_path_hist))
                        self.seg_model.train()
                        self.seg_model.reset_optimizers()
                        segmentation_loss = supervised_learning(input_image=data, input_gt=mask,segmentation_model=self.seg_model, num_classes=self.cfg.model.seg_num_classes, use_gpu=True)
                        ...
                        self.seg_model.optimize_params()
                        self.seg_model.reset_loss()
                        if  self.step != 0 and self.step % self.seg_save_and_sample_every == 0:
                            self.seg_model.save_model(self.results_folder, epoch_iter=self.step, model_prefix='seg')
                        if self.seg_model.scheduler is not None:
                            self.seg_model.scheduler.step()
                # ---------------------------------------------------------------------------------------
                    # Compute Segmentation loss
                    data1 = gen_path_data.squeeze(1)
                    data1 = (data1 - torch.min(data1)) / (torch.max(data1) - torch.min(data1))
                    mask1 = gen_path_mask.squeeze(1)
                    self.seg_model.running_metric.reset()
                    self.seg_model.eval()
                    pred1 = self.seg_model.predict(data1)
                    #seg_loss1 =  self.seg_model.get_ce_dice_loss(pred=pred1, targets=mask1, num_classes=self.cfg.model.seg_num_classes)
                    seg_loss1 = self.seg_model.get_ce_dice_loss_scar(pred=pred1, targets=mask1, num_classes=1)
                # ---------------------------------------------------------------------------------------
            loss = diff_loss + 0.01 * (seg_loss1)
            log = {'loss': loss.item()}
            ...
            if self.step != 0 and self.step % self.save_and_sample_every == 0:
                ...
                self.save(milestone)
            log_fn(log)
            print(f'Diff Loss {self.step}: {loss.item()} = {diff_loss.item()}, {seg_loss1.item()}')
            if (self.step % 10 == 0):
...
def supervised_learning(input_image, input_gt, segmentation_model, adversarial_training=False, num_classes=1, use_gpu: bool = True):
        global experiment_opt, intensity_norm_type, debug
        intensity_norm_type = 'min_max' # 'z_score'
        ...
        image_V = makeVariable(input_image.detach().clone(), type='float', use_gpu=use_gpu, requires_grad=False)
        gt_V = makeVariable(input_gt.detach().clone(), type='long',use_gpu=use_gpu, requires_grad=False)
        augmentor_opt = None
        ...
        l = segmentation_model.get_ce_dice_loss(pred=y_pred, targets=gt_V, num_classes=num_classes)
