...
import gc
from torch.utils.tensorboard import SummaryWriter
writer = SummaryWriter('LeFusion/LeFusion_Model_Joint/EMIDEC/runs/LeFusion_Model_Joint')
#--------------------------------------------------------------------------------
#--------------------------------------------------------------------------------
#--------------------------------------------------------------------------------
...
class Trainer(object):
    def __init__(
        ...
        *,
        ...
        seg_results_folder='./seg_results',
        num_sample_rows=1,
        max_grad_norm=None,
        num_workers=20,
        device=None,
        ...
        self.inf_norm_v_ds = inf_norm_v_dataset
        ...
        inf_norm_v_dl = DataLoader(self.inf_norm_v_ds, batch_size=train_batch_size, shuffle=True, pin_memory=True, num_workers=num_workers)
        ...
        self.inf_norm_v_len_dataloader = len(inf_norm_v_dl)
        ...
        self.inf_norm_v_dl = cycle(inf_norm_v_dl)
        ...
        print(f'found {len(self.inf_norm_v_ds)} normal images (for validation) as .nii.gz files')
        ...
        assert len(
            self.inf_norm_v_ds) > 0, 'need to have at least 1 image to start validation (although 1 is not great, try 1k)'
        assert len(
        ...
        self.seg_results_folder = Path(seg_results_folder)
        self.seg_results_folder.mkdir(exist_ok=True, parents=True)
        ...
        while self.step < self.train_num_steps:
            for i in range(self.gradient_accumulate_every):
                ...
                if 'hist' in path_data_frame:
                    path_hist = path_data_frame['hist'].to(self.device)
                else:
                    path_hist = None
                self.model.train()
                ...
                if (self.step >= 5000 and self.step % 10 == 0):
                    # Step 2: Infer from Lefusion (input Normal)
                    self.model.eval()
                ...
                # ---------------------------------------------------------------------------------------
                    ...
                    with autocast(enabled=self.amp):
                        ...
                        self.seg_model.is_training = True
                        step_lr = self.seg_lr_scheduler(self.seg_optimizer, epoch)
                        print('Current Step: ', self.step, ' Current Epoch: ', epoch, '  Current Learning Rate:', step_lr)
                        seg_outputs = self.seg_model(data)[0]
                        seg_outputs_soft = torch.softmax(seg_outputs, dim=1)
                        seg_outputs_pred = torch.argmax(seg_outputs, dim=1).unsqueeze(1)
                        segmentation_loss = get_ce_dice_loss_scar(pred=seg_outputs_pred, targets=mask, num_classes=2)
                        if torch.isnan(segmentation_loss):
                            print('NAN detected')
                            continue
                        #print(f'Segm Loss {self.step}: {segmentation_loss.item()}')
                        #seg_log = {'segmentation loss': segmentation_loss.item()}
                        self.seg_optimizer.zero_grad()
                        segmentation_loss.requires_grad = True
                        segmentation_loss.backward()
                        self.seg_optimizer.step()
                        # Save checkpoints
                        file_name = os.path.join(self.seg_results_folder, 'seg_model_last.pth')
                        torch.save({'epoch': epoch, 'state_dict': self.seg_model.state_dict(),'optim_dict': self.seg_optimizer.state_dict(), }, file_name)
                        if (epoch + 1) % 100 == 0 or (epoch >= (self.cfg.model.seg_num_epochs - 10)):
                            file_name = os.path.join(self.seg_results_folder, 'seg_model_{}.pth'.format(epoch + 1))
                            torch.save({'epoch': epoch, 'state_dict': self.seg_model.state_dict(), 'optim_dict': self.seg_optimizer.state_dict(),},file_name)
                        epoch = epoch + 1
                    ##################################################################################################
                    # Visualize results
                    writer.add_scalar("Train/lr", step_lr, epoch)
                    real_norm_data = torchvision.utils.make_grid(batch['GT'].squeeze(1))
                    real_norm_mask = torchvision.utils.make_grid(batch.get('gt_keep_mask').squeeze(1) == 3.0)
                    writer.add_image('(input) Infer from Lefusion: Images', real_norm_data)
                    writer.add_image('(input) Infer from Lefusion: Masks', real_norm_mask)
                    gen_norm_data = torchvision.utils.make_grid(data)
                    gen_norm_mask = torchvision.utils.make_grid(mask == 3.0)
                    pred_norm_mask = torchvision.utils.make_grid((seg_outputs_pred==3.0))
                    writer.add_image('(output) Infer from Lefusion: Images', gen_norm_data)
                    writer.add_image('(output) Infer from Lefusion: Masks', gen_norm_mask)
                    writer.add_image('(output) Infer from nnUNet: Preds', pred_norm_mask)
                    ##################################################################################################
                # ----------------------------------------------------------------------------------------------------
                if (self.step >= 5000 and self.step % 20 == 0):
                    # Step 4: Validate the segmentation model
                    self.model.eval()
                    current_dir = os.path.dirname(os.path.abspath(__file__))
                    file_path = os.path.join(current_dir, 'hist_clusters', 'emidec_clusters.json')
                    with open(file_path, 'r') as f:
                        clusters = json.load(f)
                    v_cluster_centers = clusters[0]['centers']
                    # print("sampling...")
                    with autocast(enabled=self.amp):
                        for type in range(self.cfg.model.types):
                            # print("type_of_cond:", type + 1)
                            v_in_hist = th.tensor(v_cluster_centers[type])
                            v_in_hist = perturb_tensor(tensor=v_in_hist)
                            v_in_hist = v_in_hist.unsqueeze(0)
                            v_batch = next(self.inf_norm_v_dl)
                            for k in v_batch.keys():
                                if isinstance(v_batch[k], th.Tensor):
                                    v_batch[k] = v_batch[k].to(self.device)
                            v_model_kwargs = {}
                            v_model_kwargs["gt"] = v_batch['GT']
                            v_gt_keep_mask = v_batch.get('gt_keep_mask')
                            if v_gt_keep_mask is not None:
                                v_model_kwargs['gt_keep_mask'] = v_gt_keep_mask
                            v_batch_size = v_model_kwargs["gt"].shape[0]
                            sample_fn = self.model.p_sample_loop_repaint
                            v_output = sample_fn(
                                shape=(
                                v_batch_size, self.cfg.model.diffusion_num_channels, self.cfg.model.diffusion_depth_size,
                                self.cfg.model.diffusion_img_size, self.cfg.model.diffusion_img_size),
                                model_kwargs=v_model_kwargs,
                                device=self.device,
                                progress=True,
                                conf=self.cfg,
                                cond=v_in_hist
                            )
                            v_gen_path_data = v_output.to(self.device)
                            v_gen_path_mask = v_batch['gt_keep_mask']
                            v_gen_path_hist = v_batch['hist']
                    # print("sampling complete")
                    # ---------------------------------------------------------------------------------------
                    v_data1 = np.reshape(v_gen_path_data.cpu().numpy(), (v_batch_size, self.cfg.model.diffusion_img_size, self.cfg.model.diffusion_img_size))
                    v_mask1 = np.reshape(v_gen_path_mask.cpu().numpy(), (v_batch_size, self.cfg.model.diffusion_img_size, self.cfg.model.diffusion_img_size))
                    v_data = torch.zeros((v_batch_size, 1, self.cfg.model.seg_image_height, self.cfg.model.seg_image_width)).to(self.device)
                    v_mask = torch.zeros((v_batch_size, 1, self.cfg.model.seg_image_height, self.cfg.model.seg_image_width)).to(self.device)
                    for b in range(v_batch_size):
                        v_data[b, 0, :, :] = torch.from_numpy(resize(v_data1[b, :, :], (self.cfg.model.seg_image_height, self.cfg.model.seg_image_width)))
                        v_mask[b, 0, :, :] = torch.from_numpy(cv2.resize(v_mask1[b, :, :], (self.cfg.model.seg_image_height, self.cfg.model.seg_image_width),interpolation=cv2.INTER_NEAREST))
                    v_data = (v_data - torch.min(v_data)) / (torch.max(v_data) - torch.min(v_data))
                    with torch.no_grad():
                        self.seg_model.eval()
                        self.seg_model.is_training = False
                        v_out = self.seg_model(v_data)[0]
                        v_pred = torch.argmax(v_out, dim=1)
                        dice_score = dice_score_for_scar(output=v_pred.cpu().numpy(), target=v_mask.squeeze(1).cpu().numpy())
                        writer.add_scalar("Valid/dice_score", dice_score, self.step)
                    ##################################################################################################
                    # Visualize results
                    real_norm_v_data = torchvision.utils.make_grid(v_batch['GT'].squeeze(1))
                    real_norm_v_mask = torchvision.utils.make_grid(v_batch.get('gt_keep_mask').squeeze(1) == 3.0)
                    writer.add_image('(valid input) Infer from Lefusion: Images', real_norm_v_data)
                    writer.add_image('(valid input) Infer from Lefusion: Masks', real_norm_v_mask)
                    gen_norm_v_data = torchvision.utils.make_grid(v_data)
                    gen_norm_v_mask = torchvision.utils.make_grid(v_mask == 3.0)
                    pred_norm_v_mask = torchvision.utils.make_grid((v_pred == 3.0).unsqueeze(1))
                    writer.add_image('(valid output) Infer from Lefusion: Images', gen_norm_v_data)
                    writer.add_image('(valid output) Infer from Lefusion: Masks', gen_norm_v_mask)
                    writer.add_image('(seg valid output) Infer from nnUNet: Preds', pred_norm_v_mask)
                    ##################################################################################################
                # ----------------------------------------------------------------------------------------------------
            if (self.step < 5000):
