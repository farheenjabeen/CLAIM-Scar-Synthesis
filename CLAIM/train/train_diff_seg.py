...
from get_dataset.get_dataset import get_train_dataset_path, get_train_dataset_norm, get_inference_dataloader_path, get_inference_dataloader_norm, get_inference_dataloader_norm_valid
import torch
...
def run(cfg: DictConfig):
    ...
    seg_lr_scheduler = LR_Scheduler(cfg.model.seg_lr, cfg.model.seg_num_epochs, warmup=0, mode='poly')
    train_params = [{'params': segmentor.parameters(), 'lr': cfg.model.seg_lr, 'weight_decay': cfg.model.seg_weight_decay}]
    seg_optimizer = torch.optim.SGD(train_params, momentum=0.99, nesterov=True)
    ...
    norm_v_inf_dataset = get_inference_dataloader_norm_valid(cfg)
    ...
    trainer = Trainer(
        ...
        seg_results_folder=cfg.model.seg_results_folder,
