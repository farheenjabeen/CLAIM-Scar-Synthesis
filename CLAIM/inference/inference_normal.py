...
from ddpm import Unet3D, GaussianDiffusion_Nolatent_Normal
...
def main(conf: DictConfig):
    ...
    if data_type not in ['lidc', 'emidec', 'emidec_normal']:
    ...
    diffusion = GaussianDiffusion_Nolatent_Normal(
        model,
        ...
        for type in range(conf.types):
            ...
            if data_type == 'lidc':
                ...
                for i in range(batch_size):
                    ...
                    label = batch['gt_keep_mask'][i].permute(0, 2, 3, 1).cpu()
                    label = tio.LabelMap(tensor=label, channels_last=False, affine=restore_affine)
